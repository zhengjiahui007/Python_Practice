...
def gy_main(videopath, color=None, iscmd=True):
...
def gy_read_img(file_path:str):
    # 读取图像
    img = cv2.imread(file_path)  # BGR 格式
...
if __name__ == '__main__':
    ...
    # gy_main(args.file, color=args.color)
    gy_read_img(args.file)


"""
    try:
        gy_parse = argparse.ArgumentParser()
        gy_parse.add_argument("filename",type = int,help = "Show the file name !")
        gy_parse.add_argument("filepath",help = "Show the file path !")
        gy_parse.add_argument("Author",help = "Show the file Author !")
        gy_parse.add_argument("-d","-ddf",help = "Show the optional paramters !")
        gy_parse.add_argument("--date",help = "Show the optional paramters !")

        gy_args = gy_parse.parse_args()
        print("The file name is ",gy_args.filename)
        print("The file path is {} ".format(gy_args.filepath))
        print("The Author is {} ".format(gy_args.Author))
        print("The optional is {} ".format(gy_args.d))
        print("The date is {} ".format(gy_args.date))
